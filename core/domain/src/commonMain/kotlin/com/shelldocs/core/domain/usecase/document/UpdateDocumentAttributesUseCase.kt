...
class UpdateDocumentAttributesUseCase(private val documentRepository: DocumentRepository) {
    ...
    suspend operator fun invoke(
        ...
        if (!RolePermissions.isGranted(actorRole, Permission.EDIT_DOCUMENTS)) {
            return DomainResult.failure(AppError.Unauthorized("Your role cannot edit document attributes"))
        }
        return documentRepository.updateAttributes(id, attributes)
    }
}
