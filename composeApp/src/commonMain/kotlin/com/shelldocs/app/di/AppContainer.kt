...
class AppContainer(
    ...
    // Read path: API direct when configured (the backend already is the
    // Supabase-backed source of truth, and it's the one that imports
    // Confluence pages — matched by source_external_id, deduped by content
    // hash — so there is nothing left for the client to mirror here).
    private val documentRepository: DocumentRepository by lazy {
        ...
        when {
            ...
            supabase != null -> CachingDocumentRepository(
